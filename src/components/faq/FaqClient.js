  ...
  return (
    <div>
        ...
        {FAQ_DATA.map((faq, idx) => {
          ...
          return (
            ...
            >
              ...
              >
                <span style={{ fontFamily: 'var(--font-heading)', fontSize: '1.05rem', color: 'var(--text-dark)', fontWeight: '600', paddingRight: '16px' }}>
                  {faq.q}
                </span>
                <span style={{ fontSize: '1.2rem', color: 'var(--primary-gold)', transform: isOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s ease', flexShrink: 0 }}>
                  ▼
                </span>
              </button>

              {isOpen && (
                <div style={{ padding: '0 24px 20px 24px', borderTop: '1px solid var(--bg-cream-dark)' }}>
                  <p style={{ fontSize: '0.92rem', color: 'var(--text-muted)', lineHeight: '1.7', margin: '14px 0 0 0' }}>
                    {faq.a}
                  </p>
                </div>
              )}
      ...
      <div style={{ marginTop: '3.5rem', textAlign: 'center', padding: '32px', background: 'white', borderRadius: '12px', border: '1px solid var(--primary-gold-border)' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem', maxWidth: '550px', margin: '0 auto 20px auto' }}>
          Our Jaipur master craftsmen and patron support team are available on WhatsApp and call.
        </p>
